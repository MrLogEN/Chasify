...
@Controller
public class EditProjectTaskController extends BaseController{
...
@Autowired
    ...
    public void sendTask(Task task) {
        ...
        this.task = getTaskService().findTaskById(task.getId());
        if(task != null) {
            taskId.setText(String.valueOf(task.getId()));
            setProject(task.getProject());
            getTaskName().setText(task.getName());
            setSelectedUser(task.getAssignee());
            List<User> candidates = getTaskService().getAssigneeCandidatesWithoutAssignee(task.getProject(), task.getAssignee());
            FXMLUtils.setUserListCellFactory(getUsersList());
            reloadSelectedUserLabel();
            setUserList(candidates);

            getTaskDescription().setText(task.getDescription());
            getDueDatePicker()
                .setValue(task.getDueDate() != null ? task.getDueDate().toLocalDate() : null);

            getTimeEstimateSpinner()
                .getValueFactory().setValue(
                   (double) task.getTimeEstimate().toHours());

            getStatusComboBox().setValue(task.getStatus());
        }
    }
    ...
    private void handleEdit() {
        ...
        String name = getTaskName().getText();
        String desc = getTaskDescription().getText();
        Status status = getStatusComboBox().getValue();
        User assignee = getSelectedUser();
        double timeEstimate = getTimeEstimateSpinner().getValue();

        if(assignee == null) {
            FXMLUtils.setErrorMessage(getMessageLabel(), "You must select an assignee!");
            return;
        }
        if(name == null || name.isBlank()) {
            FXMLUtils.setErrorMessage(getMessageLabel(), "Name cannot be blank!");
            return;
        }
        ...
        task.setDueDate(offsetDueDate);
        task.setTimeEstimate(estimate);
        task.setAssignee(assignee);
        getTaskService().updateTask(task);

        Stage stage = (Stage) getTimeEstimateSpinner().getScene().getWindow();
